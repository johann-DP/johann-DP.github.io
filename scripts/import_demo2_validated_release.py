...
def _source_attestation(approval: Mapping[str, Any]) -> dict[str, Any]:
...
def _verify_source_bundle(bundle_root: Path) -> VerifiedSourceBundle:
    ...
    source = approval["source"]
    expected_attestation = _canonical_json_bytes(_source_attestation(approval))
    if payloads[ATTESTATION_PATH] != expected_attestation:
        raise _fail("SITE_RELEASE_SOURCE_ATTESTATION_DIVERGED")
    ...
    if (
        ...
        or source_identity["protocol_version"] != 1
        or source_identity["approval_sha256"] != _sha256(approval_payload)
...
def _public_bundle(root: Path) -> tuple[bytes, dict[str, bytes]]:
    ...
    for item in selected:
        ...
        if (
            ...
            raise _fail("SITE_RELEASE_PUBLIC_INVALID")
    _validate_public_html(selected, payloads)
    if payloads[ATTESTATION_PATH] != _canonical_json_bytes(
        _source_attestation(approval)
    ):
        raise _fail("SITE_RELEASE_PUBLIC_INVALID")
